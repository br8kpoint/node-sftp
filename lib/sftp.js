    ...
    function parseListing(lines) {
        ...
        lines.forEach(function(line) {
            ...
            if (!line || /\/[\.]{1,2}$/.test(line))
                return;
            // Added extra reg ex as Windows permission sometimes list attributes 
            // like "?rw-rw----" 
            // This causes problems as node can't list files correctly even though they exist
            var m = line.match(/^[\s]*([drwx-]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\w\s\d]*)([\d]{2}\:[\d]{2})[\s]*(.*)$/);
            if (!m) {
                var m = line.match(/^[\s]*([?rwx-]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\d]+)[\s]*([\w\s\d]*)([\d]{2}\:[\d]{2})[\s]*(.*)$/);
                if (!m) {
                    return;
                }
            }
